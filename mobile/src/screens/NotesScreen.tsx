...
import type { NotesStackParamList, TabParamList } from "../navigation";
import { useTheme } from "../theme";
import type { NoteSummary } from "../types";

/** A note opens inside the NFs stack; the server is changed over on the Settings tab. */
type Navigation = CompositeNavigationProp<
  NativeStackNavigationProp<NotesStackParamList, "NoteList">,
      ...
      {state.status === "loading" ? (
        ...
        <View style={styles.center}>
          ...
          <Pressable onPress={changeServer} accessibilityRole="button" hitSlop={8}>
            <Text style={[styles.linkText, { color: theme.accent }]}>{t.notes.changeServer}</Text>
          </Pressable>
        </View>
      ) : (
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Text style={[styles.emptyTitle, { color: theme.text }]}>{t.notes.emptyTitle}</Text>
              <Text style={[styles.centerText, { color: theme.muted }]}>{t.notes.emptyBody}</Text>
            </View>
          }
        />
      )}
